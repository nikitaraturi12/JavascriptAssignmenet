...
const rl = readline.createInterface({
  input: fs.createReadStream('../Data/Agriculture.csv')
});
rl.on('line', (line) => {
	arrayOne.push(line);
  })
  ...
  function oilseedsProduction()
  {
    ...
    //Extracting the required data from the csv file
        ...
        var count3=(firstCol.match(/Production/g) || []).length;
          ...
          console.log("/*------------------------------------------------------------------------------------------------------*/");
          fs.writeFileSync('../output/OilseedsVsProduction.json',JSON.stringify(firstArray));//creating JSON
    ...
    for(var i=0;i<noOfRow;i++)
    {
      ...
      if(count>0)
      {
        ...
        if(count1==0 && count2==0 && count3==0 && count4==1 && count5==0)
        {
          ...
          console.log("/*----------------------------------------------------------------------------------------------*/");
          fs.writeFileSync('../output/FoodgrainsVsProduction.json',JSON.stringify(secondArray));//creating JSON
  ...
  function commercialCrops()
  {
    ...
    for(var j=year1;j<=yearN;j++)
    {
      ...
      thirdArray.push(tempObject);
        ...
        console.log("/*--------------------------------------------------------------------------------------------------------*/");
        fs.writeFileSync('../output/AggregateCommercialCropsVsYear.json',JSON.stringify(thirdArray));//creating JSON
    ...
    for(var i=firstYear;i<lastYear;i++)
    {
     ...
     for(var j=0;j<noOfRow;j++)
     {
        ...
        fourthArray.push(fourObject);
          ...
          console.log("/*------------------------------------------------------------------------------------------------------*/");
          fs.writeFileSync('../output/RiceProductionOfSouthernStates.json',JSON.stringify(fourthArray));//creating JSON
